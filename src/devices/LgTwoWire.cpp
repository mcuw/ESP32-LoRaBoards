...
void scanDevices(TwoWire *w)
{
    ...
    ESP_LOGD(TAG, "I2C Devices scanning");
    for (addr = 1; addr < 127; ++addr)
    {
        ...
        if (err == 0)
        {
            ...
            case 0x77:
                bme280_address = addr;
                ESP_LOGD(TAG, "\tFound BME280 Sensor at address 0x%02X", addr);
            ...
            case 0x76:
                ...
                break;
            case 0x34:
                ESP_LOGD(TAG, "\tFound AXP192/AXP2101 PMU at address 0x%02X", addr);
            ...
            case 0x3D:
            {
                ...
                if (r == 0x80)
                {
                    ESP_LOGD(TAG, "\tFound QMC6310N MAG Sensor at address 0x%02X", addr);
                    mag_address = addr;
                    LgLoraBoard::updateOnlineStatus(QMC6310N_ONLINE);
                }
                else
                {
                    ESP_LOGD(TAG, "\tFound OLED display at address 0x%02X", addr);
            ...
            break;
            case 0x51:
                ESP_LOGD(TAG, "\tFound PCF8563 RTC at address 0x%02X", addr);
                LgLoraBoard::updateOnlineStatus(PCF8563_ONLINE);
                break;
            case 0x1C:
                ESP_LOGD(TAG, "\tFound QMC6310U MAG Sensor at address 0x%02X", addr);
                LgLoraBoard::updateOnlineStatus(QMC6310U_ONLINE);
                mag_address = addr;
                break;
            default:
                ESP_LOGD(TAG, "\tI2C device found at address 0x%02X", addr);
                break;
            }
        }
        else if (err == 4)
        {
            ESP_LOGD(TAG, "Unknown error at address 0x%02X", addr);
            if (addr < 16)
            {
                ESP_LOGD(TAG, "0");
            }
            ESP_LOGD(TAG, "%02X", addr);
        }
    }
    if (nDevices == 0)
